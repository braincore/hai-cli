...
pub fn ai_model_from_string(ai_model: &str) -> Option<AiModel> {
    ...
    match model_name.as_str() {
        ...
        "geminipro" | "gemini31pro" => Some(AiModel::Google(GoogleModel::Gemini31Pro(
            parse_gemini_opts(opts),
        ))),
        ...
        "gpt53chat" | "g53chat" | "g53c" | "53chat" | "53c" => {
            Some(AiModel::OpenAi(OpenAiModel::Gpt53Chat))
        }
        "gpt54" | "g54" => Some(AiModel::OpenAi(OpenAiModel::Gpt54(parse_gpt5_opts(
            opts, false,
        )))),
        "gpt54mini" | "g54mini" | "g54m" | "54m" => Some(AiModel::OpenAi(OpenAiModel::Gpt54Mini(
            parse_gpt5_opts(opts, false),
        ))),
...
pub static AUTOCOMPLETE_AI_MODEL_SUGGESTIONS: &[&str] = &[
    "41",
    "41m",
    "41n",
    "4o",
    ...
    match ai_model {
        ...
        AiModel::Google(model) => match model {
            ...
            GoogleModel::Gemini31FlashLite(opts) => {
                let base = "gemini-3.1-flash-lite".to_string();
                append_gemini_opts(base, opts)
            }
            GoogleModel::Gemini31Pro(opts) => {
                let base = "gemini-3.1-pro".to_string();
                append_gemini_opts(base, opts)
            }
        ...
        AiModel::OpenAi(model) => match model {
            ...
            OpenAiModel::Gpt53Chat => "gpt-5.3-chat".to_string(),
            OpenAiModel::Gpt54(opts) => {
                let base = "gpt-5.4".to_string();
                append_gpt5_opts(base, opts)
            }
            OpenAiModel::Gpt54Mini(opts) => {
                let base = "gpt-5.4-mini".to_string();
                append_gpt5_opts(base, opts)
            }
            OpenAiModel::Gpt54Nano(opts) => {
                let base = "gpt-5.4-nano".to_string();
                append_gpt5_opts(base, opts)
            }
...
pub fn get_ai_model_provider_name(ai_model: &AiModel) -> &str {
    match ai_model {
        ...
        AiModel::OpenAi(model) => match model {
            ...
            OpenAiModel::Gpt53Chat => "gpt-5.3-chat-latest",
            OpenAiModel::Gpt54(_) => "gpt-5.4-2026-03-05",
...
pub fn get_ai_model_display_name(ai_model: &AiModel) -> String {
    match ai_model {
        ...
        AiModel::Google(model) => match model {
            ...
            GoogleModel::Gemini31FlashLite(opts) => {
                format!("gemini-3.1-flash-lite{}", get_gemini_opts_display(opts))
            }
            GoogleModel::Gemini31Pro(opts) => {
                format!("gemini-3.1-pro{}", get_gemini_opts_display(opts))
            }
            GoogleModel::Gemini35Flash(opts) => {
        ...
        AiModel::OpenAi(model) => match model {
            ...
            OpenAiModel::Gpt52Chat => "gpt-5.2-chat".to_string(),
            OpenAiModel::Gpt53Chat => "gpt-5.3-chat".to_string(),
            OpenAiModel::Gpt54(opts) => format!("gpt-5.4{}", get_gpt5_opts_display(opts)),
...
pub fn get_ai_model_cost(ai_model: &AiModel) -> Option<(u32, u32)> {
    match ai_model {
        ...
        AiModel::OpenAi(model) => match model {
            ...
            OpenAiModel::Gpt53Chat => Some((1750, 14000)),
            OpenAiModel::Gpt54(_) => Some((2500, 15000)),
