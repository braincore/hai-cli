...
pub async fn get_symmetric_key_ez(
    ...
    enc_aes_key_hex: &str,
    enc_key_id: &str,
) -> Result<SymmetricKeyInfo, GetPerFileDecryptionInfoError> {
    ...
    let password = if let Some(password) =
        term::ask_question(&format!("Enter password (key={enc_key_id}):"), true)
    {
        password
    } else {
...
pub async fn get_per_file_symmetric_key_from_metadata_ez(
    ...
    api_client: &HaiClient,
    md_contents: &[u8],
) -> Result<Option<SymmetricKeyInfo>, GetPerFileDecryptionInfoError> {
