...
//
// Content Encryption/Decryption with chunking
//

const NONCE_LEN: usize = 12;
const TAG_LEN: usize = 16;

/// Per-chunk overhead = 4 bytes (length) + 16 bytes (tag) = **20 bytes**
/// | Chunk Size | Overhead per GiB | Overhead % |
/// |------------|------------------|------------|
/// | 64 KiB     | 320 KiB          | 0.03%      |
/// | 1 MiB      | 20 KiB           | 0.002%     |
/// | 4 MiB      | 5 KiB            | 0.0005%    |
/// | 16 MiB     | 1.25 KiB         | 0.0001%    |
const DEFAULT_CHUNK_SIZE: usize = 1024 * 1024; // 1 MiB

#[derive(Clone, Debug)]
pub struct EncryptedContent {
    /// Base nonce (12 bytes); each chunk derives its nonce from this
    pub nonce: [u8; 12],
    /// Plaintext chunk size (ciphertext chunks are this + 16 bytes for tag)
    pub chunk_size: u32,
    /// All chunks concatenated: [len1][ct1][len2][ct2]...
    /// Each chunk: 4-byte LE length prefix + ciphertext (includes GCM tag)
    pub ciphertext: Vec<u8>,
}

impl EncryptedContent {
    /// Serializes to bytes: `[nonce || chunk_size || ciphertext]`
    ///
    /// # Returns
    ///
    /// Byte vector containing the full serialized format (minimum 16 bytes for empty content)
    pub fn to_bytes(&self) -> Vec<u8> {
        let mut out = Vec::with_capacity(NONCE_LEN + 4 + self.ciphertext.len());
        out.extend_from_slice(&self.nonce);
    ...
    pub fn from_bytes(bytes: &[u8]) -> Result<Self, CryptoError> {
        const HEADER_LEN: usize = NONCE_LEN + 4;
        const TAG_LEN: usize = 16;
        ...
        let chunk_size = u32::from_le_bytes(
            bytes[NONCE_LEN..HEADER_LEN]
                .try_into()
                .map_err(|_| CryptoError::InvalidFormat)?,
        );

        // Sanity check: chunk_size should be reasonable (e.g., 1 KiB to 64 MiB)
        ...
        // Optional: validate that ciphertext structure is plausible
        // Each chunk needs at least 4 (len) + 16 (tag) = 20 bytes minimum
        // But empty content (0 chunks) is valid, so only check if non-empty
        if !ciphertext.is_empty() && ciphertext.len() < 4 + TAG_LEN {
            return Err(CryptoError::InvalidFormat);
        }
...
/// Encrypts arbitrary content using chunked AES-256-GCM.
///
/// Each chunk is independently authenticated, enabling streaming decryption
/// with per-chunk verification for large data.
///
/// # Arguments
///
/// * `plaintext` - The data to encrypt (any length)
/// * `key` - The AES-256 key
/// * `chunk_size` - Optional chunk size in bytes (default: 1 MiB)
///
/// # Returns
///
/// * `Ok(EncryptedContent)` - Encrypted content with all chunks
...
pub fn encrypt_content(plaintext: &[u8], key: &AesKey) -> Result<EncryptedContent, CryptoError> {
    ...
    // Pre-allocate output buffer (estimate: input + 4 bytes + 16 tag per chunk)
    let num_chunks = (plaintext.len() + chunk_size - 1) / chunk_size.max(1);
    let estimated_size = plaintext.len() + (num_chunks * (4 + TAG_LEN));
    let mut ciphertext_out = Vec::with_capacity(estimated_size);
    ...
    while offset < plaintext.len() {
        ...
        // Write length prefix (4 bytes LE) + ciphertext
        let len = chunk_ciphertext.len() as u32;
        ciphertext_out.extend_from_slice(&len.to_le_bytes());
        ciphertext_out.extend_from_slice(&chunk_ciphertext);
...
///
/// Processes chunk-by-chunk, verifying each chunk's authentication tag.
/// Fails immediately if any chunk fails authentication.
///
/// # Arguments
///
/// * `encrypted` - The chunked encrypted content
/// * `key` - The AES-256 key used for encryption
///
/// # Returns
///
/// * `Ok(Vec<u8>)` - Successfully decrypted plaintext
/// * `Err(CryptoError::DecryptionFailed)` - If any chunk fails decryption/auth
pub fn decrypt_content(encrypted: &EncryptedContent, key: &AesKey) -> Result<Vec<u8>, CryptoError> {
    ...
    let base_nonce = &encrypted.nonce;
    let data = &encrypted.ciphertext;

    // Estimate output size (ciphertext - overhead)
    let mut plaintext_out = Vec::with_capacity(data.len());
    ...
    while offset < data.len() {
        ...
        let chunk_ciphertext = &data[offset..offset + len];
        offset += len;
        ...
        plaintext_out.extend_from_slice(&chunk_plaintext);
        chunk_index += 1;
    }
...
/// Derives a unique nonce for each chunk by XORing base nonce with chunk index.
fn derive_chunk_nonce(base_nonce: &[u8; NONCE_LEN], chunk_index: u64) -> [u8; NONCE_LEN] {
    let mut nonce = *base_nonce;
    let index_bytes = chunk_index.to_le_bytes(); // 8 bytes
...
//
// Encrypting & decrypting chunked files with streaming
//

#[allow(dead_code)]
/// Encrypts a file using chunked AES-256-GCM, streaming to output file.
///
/// NOTE: Not in use since the server-side API doesn't have much of a streaming
/// interface yet.
pub fn encrypt_file(
    in_path: &Path,
    out_path: &Path,
    ...
    let mut reader =
        BufReader::new(File::open(in_path).map_err(|_| CryptoError::EncryptionFailed)?);
    let mut writer =
        BufWriter::new(File::create(out_path).map_err(|_| CryptoError::EncryptionFailed)?);

    // Write header: nonce + chunk_size
    writer
    ...
    loop {
        let bytes_read = read_full_or_eof(&mut reader, &mut plaintext_buf)
            .map_err(|_| CryptoError::EncryptionFailed)?;
        ...
        let ciphertext = cipher
            .encrypt(nonce, &plaintext_buf[..bytes_read])
            .map_err(|_| CryptoError::EncryptionFailed)?;
    ...
    writer.flush().map_err(|_| CryptoError::EncryptionFailed)?;
    Ok(())
}

/// Decrypts a file that was encrypted with [`encrypt_file_chunked`].
pub fn decrypt_file(in_path: &Path, out_path: &Path, key: &AesKey) -> Result<(), CryptoError> {
    let cipher =
        Aes256Gcm::new_from_slice(key.as_bytes()).map_err(|_| CryptoError::DecryptionFailed)?;

    let mut reader =
        BufReader::new(File::open(in_path).map_err(|_| CryptoError::DecryptionFailed)?);
    let mut writer =
        BufWriter::new(File::create(out_path).map_err(|_| CryptoError::DecryptionFailed)?);
    ...
    let max_ct_len = chunk_size + TAG_LEN;
    let mut ciphertext_buf = vec![0u8; max_ct_len];
    let mut chunk_index: u64 = 0;
    ...
    loop {
        ...
        reader
            .read_exact(&mut ciphertext_buf[..ct_len])
            .map_err(|_| CryptoError::DecryptionFailed)?;
        ...
        let plaintext = cipher
            .decrypt(nonce, &ciphertext_buf[..ct_len])
            .map_err(|_| CryptoError::DecryptionFailed)?;
    ...
    writer.flush().map_err(|_| CryptoError::DecryptionFailed)?;
    Ok(())
}

/// Reads exactly `buf.len()` bytes or less if EOF is reached.
