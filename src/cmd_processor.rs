...
pub async fn process_cmd(
    ...
    match cmd.clone() {
        cmd::Cmd::Noop => ProcessCmdResult::loop_next(),
        cmd::Cmd::Quit => {
            outln!(io, "さようなら！");
...
"##
                ...
                if let Some((ref task_fqn, ref task_key, step_index)) = task_step_signature {
                    ...
                    if let Some(cached_output) = cached_output {
                        ...
                        if !force_yes && !trusted {
                            outln!(io);
                            let answer = io
                                .prompt(&crate::io::Prompt::line("Execute above command? y/[n]:"))
                                .into_option()
                                .unwrap_or_default();
        ...
        cmd::Cmd::AskHuman(cmd::AskHumanCmd {
            ...
            let prompt = crate::io::Prompt::line(question)
                .with_secret(secret)
            ...
            let (answer, from_cache) = if cache {
                if let Some((ref task_fqn, ref task_key, step_index)) = task_step_signature {
                    ...
                    .map(|a| (Some(a), true))
                    .unwrap_or_else(|| (io.prompt(&prompt).into_option(), false))
                } else {
                    (io.prompt(&prompt).into_option(), false)
                }
            } else {
                (io.prompt(&prompt).into_option(), false)
            };
        ...
        | cmd::Cmd::Pin(cmd::PinCmd { message, .. }) => {
            ...
            ProcessCmdResult::loop_next()
                ...
                .with_history_entries(vec![HistoryEntry::UserText(message)])
        }
        ...
        cmd::Cmd::Task(cmd::TaskCmd {
            ...
            .await
            {
                ...
                outln!(io);
                // FIXME: Remove black().on_white() [add banner mode?]
                outln!(
                    io,
                    "{} {}",
        ...
        cmd::Cmd::AssetRemove(cmd::AssetRemoveCmd {
                ...
                && !asset_reader::is_glob_pattern(&asset_name)
            {
                ...
                let Some(answer) = io.prompt(&crate::io::Prompt::confirm(q)).into_option() else {
                    return ProcessCmdResult::loop_next();
                };
        ...
        cmd::Cmd::AssetCryptRecover(cmd::AssetCryptRecoverCmd { enc_key_id }) => {
            ...
            let recovery_code_str = if let Some(code) = io
                .prompt(&crate::io::Prompt::line("Enter recovery code:"))
                .into_option()
            {
                ...
                .into_option()
            {
                ...
                let password_verify = io
                    .prompt(&crate::io::Prompt::secret_line("Verify new password:"))
                    .into_option();
        ...
        cmd::Cmd::AccountNew => {
            ...
            loop {
                username = match io
                    .prompt(&crate::io::Prompt::line("Username?"))
                    .into_option()
                {
            ...
            loop {
                password = match io
                    .prompt(&crate::io::Prompt::secret_line("Password?").with_record_answer(false))
                    .into_option()
                {
            ...
            let terms_answer = match io
                .prompt(&crate::io::Prompt::confirm(
