...
pub enum CmdSource {
    ...
    /// bool is whether to to print cmd
    Internal(bool),
...
#[derive(Debug)]
pub struct CmdGroup {
    pub cmds: VecDeque<CmdInput>,
    pub cascade_error: bool,
}

#[derive(Debug)]
pub struct CmdQueue {
    groups: VecDeque<CmdGroup>,
}
...
impl CmdQueue {
    ...
    pub fn push_group(&mut self, group: CmdGroup) {
        self.groups.push_front(group);
    }

    pub fn push_cmds(&mut self, cmds: Vec<CmdInput>, cascade_error: bool) {
        if !cmds.is_empty() {
            self.groups.push_front(CmdGroup {
    ...
    pub fn push_cmd(&mut self, cmd: CmdInput) {
        self.push_cmds(vec![cmd], false);
    }

    /// Pop the next command to run.
    ///
    /// Implementation wise, this will pop from a group potentially leaving it
    /// empty. This is necessary so that `fail_current` can "drop the current"
    /// group on failure.
    ///
    /// The empty group will be discarded on the next call.
    pub fn next_cmd(&mut self) -> Option<CmdInput> {
        while let Some(front) = self.groups.front_mut() {
            if let Some(cmd) = front.cmds.pop_front() {
                return Some(cmd);
            }
    ...
    /// Call this when the command returned by `next_cmd` failed.
    /// Drops the rest of the current group if it cascades.
    /// Returns true if remaining commands were skipped.
    pub fn fail_current(&mut self) -> bool {
        match self.groups.front() {
            Some(g) if g.cascade_error => {
                self.groups.pop_front();
                true
            }
            _ => false,
    ...
    pub fn is_empty(&self) -> bool {
        self.groups.iter().all(|g| g.cmds.is_empty())
    }

    /// Retain only the commands matching the predicate, across all groups.
    /// Groups left empty are discarded.
    pub fn retain<F>(&mut self, mut f: F)
    where
        F: FnMut(&CmdInput) -> bool,
    {
        for group in self.groups.iter_mut() {
            group.cmds.retain(|cmd| f(cmd));
        }
        self.groups.retain(|g| !g.cmds.is_empty());
    }

    /// Like `retain`, but if any command is removed from a cascading group,
    /// the entire group is dropped.
    pub fn retain_cascade_aware<F>(&mut self, mut f: F)
    where
        F: FnMut(&CmdInput) -> bool,
    {
        self.groups.retain_mut(|group| {
            let before = group.cmds.len();
            group.cmds.retain(|cmd| f(cmd));

            if group.cascade_error && group.cmds.len() != before {
                false // something was purged; kill the whole group
            } else {
                !group.cmds.is_empty()
            }
        });
...
pub struct SessionState {
    ...
    pub cmd_queue: Arc<Mutex<CmdQueue>>,
...
impl SessionState {
    pub fn new_from_cfg(
        ...
        SessionState {
            ...
            cmd_queue: Arc::new(Mutex::new(CmdQueue::new())),
